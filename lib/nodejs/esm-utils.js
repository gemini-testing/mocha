...
const forwardAsIs = p => p;

const formattedImport = async (file, esmDecorator = forwardAsIs) => {
...
async function implementationOfRequireOrImportForUnstableEsm(
  file,
  esmDecorator
) {
  if (path.extname(file) === '.mjs') {
